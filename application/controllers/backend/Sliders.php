...
class Sliders extends BackEndController {
    ...
    public function index() {
        ...
        $objects = $this->sliders_model->find();
    ...
    public function put() {
        ...
        try {
            ...
            if (!empty($_FILES['imgMob']['name'])) {
                if (strpos($_FILES['imgMob']['name'], '.mp4')) {
                    $uploaddir = realpath("public") . '/' . $this->main_page . '/video/';
                    $file = basename($_FILES['imgMob']['name']);
                    $uploadfile = $uploaddir . $file;
                    if (move_uploaded_file($_FILES['imgMob']['tmp_name'], $uploadfile)) {
                        $post['imgMob'] = $file;
                    }
                } else {
                    $this->upload->do_upload('imgMob');
                    $file_data = $this->upload->data();
                    $file = $file_data['file_name'];
                    if(verify_img_extension($file_data['file_ext']))  $post['imgMob'] = $file;
                }
            }

            if (!$this->sliders_model->put($post)) {
    ...
    public function update_order() {
        ...
        try {
            ...
            if (!$this->sliders_model->update_sorder($post)) {
                throw new Exception(lang('Error writing data to table') . $this->main_page);
            }
    ...
    public function item($id = 0) {
        ...
        init_load_img($this->main_page);
        $item = $this->sliders_model->find_first($id);
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            try {
                ...
                if (!empty($_FILES['video']['name'])) {
                    if (strpos($_FILES['video']['name'], '.mp4')) {
                        $uploaddir = realpath("public") . '/' . $this->main_page . '/';
                ...
                if (!$this->sliders_model->update($post, $id)) {
                    throw new Exception(lang('Error writing data to table') . $this->main_page);
                }
    ...
    public function delete($id = false) {
        ...
        try {
            if (!$this->sliders_model->delete($id)) {
