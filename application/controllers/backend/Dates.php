...
class Dates extends BackEndController
{
    protected $add;
    ...
    public function index()
    {
        ...
        $objects = $this->dates_model->find('id DESC');
    ...
    public function update_order()
    {
        ...
        try {
            ...
            if (!$this->dates_model->update_sorder($post)) {
                throw new Exception(lang('Error writing data to table') . $this->main_page);
            }
    ...
    public function item($id = 0)
    {
        ...
        init_load_img($this->main_page);
        $item = $this->dates_model->find_first($id);
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            try {
                ...
                if (!$this->dates_model->update($post, $id)) {
                    throw new Exception(lang('Error writing data to table') . $this->main_page);
                }
        ...
        $this->data['title'] = lang('Edit') . $item->{'name'};
