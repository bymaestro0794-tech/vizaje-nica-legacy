...
class Stores extends BackEndController
{
    protected $add;
    ...
    public function __construct()
    {
        ...
        $this->load->model('stores_img_model');
    }
    ...
    public function index()
    {
        ...
        $objects = $this->stores_model->find();
            ...
            if (isset($_FILES['images'])) {
                ...
                for ($i = 0; $i < $filesCount; $i++) {
                    ...
                    if ($this->upload->do_upload('file')) {
                        ...
                        $uploadData[$i]['stores_id'] = $id;
                        $uploadData[$i]['img'] = $fileData['file_name'];
                    }
                }

                if (!empty($uploadData)) {
                    $this->db->insert_batch('stores_img', $uploadData);
                }
            }
    ...
    public function update_order()
    {
        ...
        try {
            ...
            if (!$this->stores_model->update_sorder($post)) {
                throw new Exception(lang('Error writing data to table') . $this->main_page);
            }
    ...
    public function item($id = 0)
    {
        ...
        init_load_img($this->main_page);
        $item = $this->stores_model->find_first($id);
            ...
            try {
                ...
                if (isset($post['image_order'])) {
                    ...
                    if (!empty($image_orders)) {
                        $this->db->update_batch('stores_img', $image_orders, 'id');
                    }

                    unset($post['image_order']);
                }

                if (!$this->stores_model->update($post, $id)) {
                    throw new Exception(lang('Error writing data to table') . $this->main_page);
                }
        ...
        $item->images = $this->db->where('stores_id', $id)->order_by('sorder ASC, id DESC')->get('stores_img')->result();
        $this->data['inner_view'] = $this->item_view;
        $this->data['title'] = 'Редактирование ' . $item->{'title' . get_language_for_admin(true)};
