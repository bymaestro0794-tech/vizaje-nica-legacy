...
class Analytics extends CI_Controller
{
    ...
    private function validatePayload(array $payload)
    {
        ...
        $event = array(
            ...
            'traffic_source' => $this->nullableAttribution($payload, 'traffic_source', 80),
            'traffic_medium' => $this->nullableAttribution($payload, 'traffic_medium', 80),
            'traffic_campaign' => $this->nullableAttribution($payload, 'traffic_campaign', 120),
            'referrer_host' => $this->nullableHost($payload, 'referrer_host'),
            'device_type' => $this->nullableDeviceType($payload),
            'consent_version' => 'v1',
            'created_at' => date('Y-m-d H:i:s'),
        );

        if ($event['cart_key'] === false || $event['product_id'] === false || $event['category_id'] === false || $event['order_id'] === false || $event['quantity'] === false || $event['cart_items_count'] === false || $event['cart_value'] === false || $event['event_value'] === false || $event['currency'] === false || $event['page_path'] === false || $event['traffic_source'] === false || $event['traffic_medium'] === false || $event['traffic_campaign'] === false || $event['referrer_host'] === false || $event['device_type'] === false) {
            return false;
        }
    ...
    private function nullableAttribution(array $payload, $field, $maxLength)
    {
        if (!isset($payload[$field]) || $payload[$field] === '') {
            return null;
        }
        ...
        if (strlen($value) > $maxLength || !preg_match('/^[a-z0-9._-]+$/', $value)) {
            return false;
        }
    ...
    private function nullableHost(array $payload, $field)
    {
        if (!isset($payload[$field]) || $payload[$field] === '') {
            return null;
        }
        ...
        return preg_match('/^[a-z0-9.-]{1,191}$/', $value) ? $value : false;
    }

    private function nullableDeviceType(array $payload)
    {
        if (!isset($payload['device_type']) || $payload['device_type'] === '') {
            return null;
        }

        $value = strtolower(trim((string) $payload['device_type']));

        return in_array($value, array('mobile', 'tablet', 'desktop'), true) ? $value : false;
    }
