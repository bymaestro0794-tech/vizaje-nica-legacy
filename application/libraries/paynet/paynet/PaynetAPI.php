...
class PaynetEcomAPI
{
     ...
     * The base URL to UI
     */
    const PAYNET_BASE_UI_URL = "https://paynet.md/acquiring/setecom";        // test link:  https://test.paynet.md/acquiring/setecom
//    const PAYNET_BASE_UI_URL = "https://test.paynet.md/acquiring/setecom";        // test link:  https://test.paynet.md/acquiring/setecom
    /**
     * The base URL to UI
     */
    const PAYNET_BASE_UI_SERVER_URL = "https://paynet.md/acquiring/getecom";  // test link: https://test.paynet.md/acquiring/getecom
//    const PAYNET_BASE_UI_SERVER_URL = "https://test.paynet.md/acquiring/getecom";  // test link: https://test.paynet.md/acquiring/getecom

    /**
     * The base URL to API
     */
    // const PAYNET_BASE_API_URL = 'https://paynet.md:4448/';                     // test link: https://test.paynet.md:4446
    const PAYNET_BASE_API_URL = 'https://ecom-api.paynet.md';
//    const PAYNET_BASE_API_URL = 'https://test.paynet.md:4446/';                     // test link: https://test.paynet.md:4446
    /**
    ...
    {
        $path = '/api/order/merchant/' . $externalID;
        $params = [];
        ...
        if ($tokenReq->IsOk()) {
            $resultCheck = $this->callApi($path, 'GET', null, [], $tokenReq->Data);
            if ($resultCheck->IsOk()) {
    ...
    public function FormCreate($pRequest)
    {
        ...
        $pp_form = '<form method="POST" id="pay_form" action="' . self::PAYNET_BASE_UI_URL . '">' .
            ...
            '<input type="hidden" name="Payer.Email"   value="' . htmlspecialchars_decode($pRequest->Customer['Code']) . '"/>' .
            '<input type="hidden" name="Payer.Name"   value="' . htmlspecialchars_decode($pRequest->Customer['Name']) . '"/>' .
            '<input type="hidden" name="Payer.Surname"   value="' . htmlspecialchars_decode($pRequest->Customer['Surname']) . '"/>' .
            '<input type="hidden" name="Payer.Mobile"   value="' . htmlspecialchars_decode($pRequest->Customer['Phone']) . '"/>' .
            ...
            '<input type="hidden" name="SaleAreaCode" value="https_vizaje-nica_com"/> ' .
            '</form>';
        $result->Data = $pp_form;
        return $result;
    }
    ...
    private function callApi($path, $method = 'GET', $params = [], $query_params = [], $headers = [])
    {
        ...
        log_message('error', 'Paynet callApi URL: ' . $url); // ВРЕМЕННО
        ...
        log_message('error', 'DIAG: Paynet callApi URL: ' . $url);
        log_message('error', 'DIAG: Paynet callApi raw response: ' . var_export($json_response, true));
        log_message('error', 'DIAG: Paynet callApi curl error: ' . curl_error($ch));
        log_message('error', 'DIAG: Paynet callApi http code: ' . curl_getinfo($ch, CURLINFO_HTTP_CODE));
    ...
    public function ExternalDate($addHours = 0)
    {
        $date = strtotime("+" . $addHours . " hour");
        return date('Y-m-d', $date) . 'T' . date('H:i:s', $date);
    }
